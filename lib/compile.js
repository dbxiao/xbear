 ...
 * @author [dbxiao]
 * @data   [2015-12-3]
 * @updata [overide fis-kernel/compile; support widget and layout table]
 *
 * @demo   <widget rel="import" href="home:widget/nav/nav.html" >
 *         <layout rel="import" href="common:widget/nav/nav.html" >
 */
 ...
 * @param  {[type]}   product   [项目名称]
 * @param  {[type]}   namespace [命名空间]
 * @param  {[type]}   file      [文件信息]
 * @param  {Function} callback  [回调]
 */
function formatStaticHolder(content, product, namespace, file, callback){
    var reg = /<!-- (COMMON|PROJECT)_STATIC_(STYLESHEET|JAVASCRIPT)_PLACEHOLDER -->/ig;
    var comCssPath = "/res/"+product+"/common/pkg/common_widget.css";
    var comJsPath  = "/res/"+product+"/common/pkg/common_widget.js";

    var proCssPath = "/res/"+product+"/"+namespace+"/pkg/"+namespace+"_widget.css";
    var proJsPath  = "/res/"+product+"/"+namespace+"/pkg/"+namespace+"_widget.js";

    callback = callback || function(m, $1, $2, $3){
        if($1){
            switch(m){
                case "<!-- COMMON_STATIC_STYLESHEET_PLACEHOLDER -->":
                    m = '<link rel="stylesheet" href="'+comCssPath+'" />';
                    break;
                case "<!-- PROJECT_STATIC_STYLESHEET_PLACEHOLDER -->":
                    m = '<link rel="stylesheet" href="'+proCssPath+'" />';
                    break;
                case "<!-- COMMON_STATIC_JAVASCRIPT_PLACEHOLDER -->":
                    m = '<script type="text/javascript" charset="utf-8" src="'+comJsPath+'"></script>';
                    break;
                case "<!-- PROJECT_STATIC_JAVASCRIPT_PLACEHOLDER -->":
                    m = '<script type="text/javascript" charset="utf-8" src="'+proJsPath+'"></script>';
                    break;
                default:
                    break;
            }
    ...
    if(typeof content == "string" && file.ext.match(/\.(html|htm|md)$/gi)){
        return content.replace(reg, callback);
...
/**
 * [formatHtml 格式化]
 * @param  {[type]}   content  [编译文档对象]
 * @param  {Function} callback [回调方法]
 */
function formatHtml(content, callback){
    var reg = /(<widget|<layout)\s+[\s\S]*?["'\s\w\/\-](?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)/ig;
    var namespace = fis.config.data.namespace,
        product   = fis.config.data.product;
    ...
    callback = callback || function(m, $1, $2, $3, $4, $5, $6, $7, $8){
        ...
        if($1){
            _result = m.match(/\shref\s*=\s*('[^']+'|"[^"]+"|[^\s\/>]+)/i);

            if(_result && _result[1]){                  // common:widget/nav/nav.html
                _namespace = _result[1].split(":")[0];  // common
                _url = _result[1].split(":")[1];        // widget/nav/nav.html

                _namespace = _namespace.replace('\"','');
                _url = _url.replace('\"','')
                           .replace('?__inline', '');

                if(_namespace == namespace){ //判断编译文件namespace和使用widget namespace是否同一个
                    m = '<link rel="import" href="../../'+_url+'?__inline" />';
                }else{
                    m = '<link rel="import" href="../../../'+_namespace+"/"+_url+'?__inline" />';
                }
            }
        }
        // console.log("------------m--------------");
    ...
    if(typeof content == "string"){ //object 类型不进行replace操作
...
function standard(file){
    ...
    if(namespace == "common"){
        try{
            content = formatStaticHolder(content, product, namespace, file);
        }catch(e){
            console.log(e);
        }
    }else{ 
