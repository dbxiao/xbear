...
var CACHE_DIR;
var _ = require('../node_modules/fis3/lib/util.js');
...
function extHtml(content, callback, file) {
    var reg = /(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|video|link|object|source|widget|layout)\s+[\s\S]*?["'\s\w\/\-](?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)/ig;
    callback = callback || function(m, $1, $2, $3, $4, $5, $6, $7, $8) {
        if ($1) { //<script>
            var embed = '';
        ...
        } else if ($5) { //<img|embed|audio|video|link|object|source|widget|layout>
            var tag = $5.toLowerCase();
            if (tag === 'link' || tag==='widget' || tag=== 'layout') {
                var inline = '',
                    isCssLink = false,
                    isImportLink = false;
...
function postStandard(file) {
    ...
    if(namespace != "common"){ //common不进行跨模块编译
        try{
